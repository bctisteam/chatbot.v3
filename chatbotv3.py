...
def speak(text): #converting text to speak
    engine.say(text)
    engine.runAndWait()
...
def commands():
    ...
    if (
        ("introduce yourself" in text)
        or ("who are you" in text)
        or ("what are you" in text)
    ):
        speak(
            "Hello, my name is Chatbot. I am an AI robot created by STEAM members in 2024. You can ask me questions about the BCTI. "
        )
    ...
    if "steam lab" in text:
        speak(
            "The STEAM Lab is located on the second floor at CTI 215. There are other projects there like PuzzleBot, DrawBot, BuzzMe, and the MPS station"
        )

    if ("3d printing" in text) or ("additive" in text) or ("javelin" in text):
        speak(
            "the javelin additive manufacturing lab is located on the second floor at cti 216. you can see that there are 3d printed models, 3d printers, and a laser cutting machine."
        )

    if ("c p factory" in text) or ("festo" in text):
        speak(
            "the cp factory is located on the first floor at cti 117. there, automated guided vehicles are occasionally driving across the room to aid ."
        )

    if ("magna" in text) or ("skills" in text) or ("mechatronics" in text):
        speak(
            "the magna mechatronics skills training room is located on the second floor at cti 210. here students are able to learn to program and build small scale versions of industrial factories."
        )

    if ("atrium" in text) or ("demonstration" in text) or ("event" in text):
        speak(
            "the Main Atrium is located on the first floor at CTI 107 and the demonstration room is located CTI 108. Many event are held in these areas."
        )

    if "prototyping" in text:
        speak(
            "The Product Prototyping Facility is located in the machine shop at CTI 110. This facility contains specialized and traditional machining equipment that will enable users to bring ideas and concepts to life by transforming digital files into tangible models, prototypes and proof-of-concepts."
        )

    if "gaming" in text:
        speak(
            "The Gaming zone is located on the second floor at CTI 209. here the humber esports team train"
        )

    if "indigenous" in text:
        speak(
            "The Indigenous Cultural Marker is located on the second floor. This sculpture is a representation of our entire life and the central piece that links this life is symbolic of the spirit."
        )

    if ("kuka" in text) or ("automation" in text):
        speak(
            "The Kuka Advanced Automation Lab is located in the fourth floor at CTI 409. This lab is equipped with robotics technology dedicated to the collaboration of industry, faculty members and students on robotics systems integration applications."
        )

    if ("S E W" in text) or ("eurodrive" in text):
        speak(
            "The S E W-Eurodrive Innovation Lab is located in the second floor at CTI 221 where it demonstrates how humans, technology, and machines collaborate on the production line. The laboratory focuses on Automated Guided Vehicles for assembly, production and distribution logistics. "
        )

    if ("capstone" in text) or ("projects" in text):
        speak(
            "the capstone projects room is located on the fourth floor at CTI 215 and CTI 216. these rooms are for students to complete their industrial capstone projects."
        )

    if "sick" in text:
        speak(
            "the Sick sensor lab is located on the third floor at CTI 316 A. this lab is used to deliver training on SICK Vison Sensor Technologies. "
        )

    if ("cisco" in text) or ("digital" in text):
        speak(
            "The Cisco Digital Transformation Zone is located on the fourth floor at CTI 410. The data centre provides critical services to support a wide range of projects and innovation activities in the capacity of Internet of Things (IoT), and digital transformation. It allows for end-user project connectivity for capstone projects, applied research, and industry activities."
        )
...
def main_menu():
    click = False
    img = pygame.image.load("/home/steam/ai screen.jpg").convert()
...
# 
        ...
        if ("chatbot" in text) or ("bot" in text) or ("chat" in text):
            start_listening = datetime.datetime.now()
            done_listening = start_listening + datetime.timedelta(seconds=30)
